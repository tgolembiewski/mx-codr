...
The gate also lists errors the server logged while the suite ran (`RUNTIME01`): a test can
pass while the page behind it threw. Microflow tests (`*.test.mdl`) are NOT run by the gate:
after changing logic, `bash tests/gate.sh --stop`, then `./mxcli test tests/ -p <app>.mpr
--local`, then `bash tests/gate.sh --boot-if-needed` -- boot again even when a test fails.
...
datagrid dg (...) { column colStatus (attribute: "Status") { dropdownfilter fltStatus } }
  ...
  --   The column needs its Attribute even when it shows Content: the filter filters on it,
  --   and without one it renders "Unable to get filter store" (gate GRID01)
...
Each page, snippet, microflow or entity is **created in one script only** -- change it
there, or with `alter`, never by a second `create or modify` in a later script: whichever
of the two runs last wins, and re-running the earlier one silently undoes the later one
(precheck refuses it: `SCRIPT01`). Exec each script by its own path, one command per
script -- `for f in …; do mxcli exec mdlsource/$f.mdl` hides the scripts from precheck and
is blocked. `gate.sh --only <feature>` ends in `PASSED … not DONE`; only the full
`bash tests/gate.sh` can say DONE.
